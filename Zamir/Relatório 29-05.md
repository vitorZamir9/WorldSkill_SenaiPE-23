# 🤖 WorldSkills #23 - Mobile Robotics

## 📅 Cronograma Vigente
* **Semana:** 01
* **Período:** 23/05/2026 a 29/05/2026
* **Fase:** P0 - Baseline competitivo e setup
* **Projeto Central:** Baseline de bancada e inventário competitivo

---

## 📌 Status dos Entregáveis da Semana
De acordo com os critérios de aceitação estabelecidos para o projeto:

- [x] Repositório Git estruturado
- [x] Inventário físico do Kit Studica Lyon
- [x] Checklist elétrico e de segurança inicial
- [x] Planilha de componentes integrada
- [ ] Diários técnicos individuais atualizados
- [ ] Vídeo curto demonstrativo da bancada

---

## 📁 Histórico de Relatórios Diários
Acesse abaixo os registros detalhados de cada sessão de treinamento executada em bancada:

* [📑 Relatório Técnico - 27/05/2026 (Auditoria Física e Firmware)](./Relat%C3%B3rio%2027-05)
* [📑 Relatório Técnico - 28/05/2026 (Checklist de Segurança e Análise de Riscos)](./Relat%C3%B3rio%2028-05)
* [📑 Relatório Técnico - 29/05/2026 (Fechamento do Baseline e Demonstração)](./Relat%C3%B3rio%2029-05)

---

## 📑 Relatório de Atividades Diárias (29/05/2026)

### 📌 Resumo das Atividades do Dia
* **Validação do Inventário Competitivo:** Consolidação final de todos os dados físicos coletados para a geração do documento de auditoria formal do Kit Lyon 2024.
* **Revisão de Handoff Técnico:** Inversão completa de papéis e simulação de banca examinadora para garantir autonomia individual em conformidade com o regulamento.
* **Garantia de Evidências:** Captura de material audiovisual e publicação das planilhas em nuvem para auditoria remota imediata do treinador.

<br/>

### 📁 Sistemas e Componentes Gerenciados

<table width="100%">
  <thead>
    <tr>
      <th align="left">Módulo / Subsistema</th>
      <th align="left">Descrição das Ações Técnicas Executadas</th>
    </tr>
  </thead>
  <tbody>
    <tr>
      <td><code>Documentação/Planilha_Mestre</code></td>
      <td>Integração e versionamento da lista detalhada contendo ID, categoria, quantidade exata e status dos itens do ecossistema Studica.</td>
    </tr>
    <tr>
      <td><code>Avaliação/Handoff_Cruzado</code></td>
      <td>Arguição oral individual e testes práticos cronometrados de localização de componentes sem auxílio de terceiros.</td>
    </tr>
    <tr>
      <td><code>Mídias/Mecanismo_Evidência</code></td>
      <td>Gravação da rotina de apresentação técnica em conformidade com o tempo limite estipulado na folha de tarefas.</td>
    </tr>
  </tbody>
</table>

<br/>

### ⏳ Linha do Tempo e Evolução do Dia

* **Etapa 1 (Manhã):** Migração dos registros físicos de hardware para o ambiente digital. Publicação e validação da [Planilha Mestre de Componentes do Kit Lyon](https://docs.google.com/spreadsheets/d/1qMr31zQ2hw8hnSi5A4h-dRlAfrvzuB5P_vKWNOsAU5A/edit?usp=sharing), mitigando qualquer risco de inconsistência em caso de auditoria externa.
* **Etapa 2 (Tarde):** Execução do Simulado P0-01 (Auditoria sob pressão). Inversão real de funções da dupla e gravação do vídeo técnico demonstrando a organização impecável e prontidão operacional da bancada de trabalho.

<br/>

---

## 🎥 Demonstração Prática de Prontidão Operacional
Abaixo está o registro em vídeo apresentando os componentes do kit, os procedimentos de segurança elétrica adotados e a verificação estática do posto de trabalho:

<div align="center">
  <video src="Evid%C3%AAncias/semana01_bancada.mp4" width="60%" controls></video>
  <br/>
  <sup><i>Apresentação Técnico: Baseline de Bancada e Alinhamento Estratégico (Duração: 60-90s).</i></sup>
</div>

---

## ✅ Checklist de Validação de Hardware & Casos de Borda

* **Teste Válido (Rastreabilidade):** Sorteio de cinco componentes críticos do kit físico e localização exata de sua respectiva linha de dados, foto e ID na planilha digital em menos de 30 segundos por elemento.
* **Teste de Limite (Performance de Setup):** Cronometragem pontual do tempo de preparação física total da bancada (ferramental e periféricos), atingindo a marca ideal em conformidade com as metas competitivas semanais.
* **Teste de Borda (Handoff por Pressão):** Arguição individual surpresa sobre as tarefas desenvolvidas pelo parceiro de equipe, testando a capacidade de assumir o posto de instrumentação e explicar as pendências técnicas de imediato.

---

## 🚨 Critérios de Parada Crítica (Safe Mode)

De acordo com o plano operacional estabelecido, o treinamento deve ser interrompido imediatamente e uma issue de manutenção deve ser aberta no repositório caso qualquer um dos seguintes cenários seja detectado:
1. Bateria com avaria física visível ou tensão de circuito aberto abaixo de **12V**.
2. Mecanismo físico de Parada de Emergência (E-Stop) obstruído ou botões de Start/Stop sem identificação clara.
3. Fusíveis de proteção da placa controladora Titan Quad rompidos, ausentes ou substituídos por especificações incorretas.
4. Cabos elétricos ou chicotes de potência sem etiquetas de identificação de origem e destino.

---
<p align="center"><i>Diário Técnico desenvolvido sob as diretrizes de alta performance da Skill #23 - WorldSkills.</i></p>
